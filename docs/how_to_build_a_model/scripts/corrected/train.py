...
# the directory that contains transforms.py and model.py. The PR used absolute
# package paths (docs.how_to_build_a_model.scripts.transforms), which only work
# when run as a module from the repo root. Pick whichever matches how you run it,
# but keep train.py, dataset_and_loader.py, and the tutorial consistent.
...
# BUGFIX: TENSOR_KEYS was referenced by collate_fn and build_pipeline in the PR
# but never defined, which raised NameError at runtime. Define it here.
TENSOR_KEYS = [
    "atomic_numbers",
    "input_coords",
    "target_coords",
