...
def crop_to_pocket(
    ...
    # BUGFIX: the original PR computed `is_ligand` but never attached it to the
    # cropped AtomArray, so FeaturizeForDocking.check_input / featurize_for_docking
    # would fail (no `is_ligand` annotation). Set the annotation before returning.
